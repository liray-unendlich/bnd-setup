...
# 変更1: CUDA イメージをやめて、素の Ubuntu に
ARG CUDA_IMG=ubuntu:24.04
ARG CUDA_RUNTIME_IMG=ubuntu:24.04
...
# Install RISC0 and groth16 component early for better caching（PreflightではCUDA不要のため残してOK）
...
# Main prover（Preflight想定だが構成はそのまま）
